...
async function request({ providerId, apiKey, baseUrl, payload, signal, stream }) {
  ...
  try {
    ...
    throw new ProviderError(providerId, `Network error contacting ${providerId}.`, { cause: e });
  }
  if (!res.ok) {
...
export async function generateChatCompletion({ providerId, apiKey, baseUrl, payload, signal }) {
  const res = await request({ providerId, apiKey, baseUrl, payload, signal, stream: false });
...
export async function* streamChatCompletion({ providerId, apiKey, baseUrl, payload, signal }) {
  const res = await request({ providerId, apiKey, baseUrl, payload, signal, stream: true });
  if (!res.body) {
    throw new ProviderError(providerId, `${providerId} returned no response body to stream.`);
  }
  ...
  try {
    ...
    throw new ProviderError(providerId, `${providerId} stream interrupted.`, { cause: e });
  } finally {
    try {
      reader.releaseLock();
    } catch {
