...
/* ── data source ─────────────────────────────────────────────────────────────
 * The package catalog is no longer hardcoded here. It lives in the CMS-managed
 * `sales_packages` Supabase table and is loaded (per tenant) into a registry via
 * `getPackageRegistry()` / `loadCompanyPackages()` in
 * `src/lib/supabase/salesPackages.js`. This module remains the pure, name-agnostic
 * engine (factories + selectors); only the data source moved. Avenix's original
 * catalog was seeded verbatim (supabase/seed/sales_packages_avenix.sql).
 * ────────────────────────────────────────────────────────────────────────── */
